...
async function extract(orders, sortBy, filter){
    let validSorts = ['dateOrdered_asc', 'dateOrdered_dsc'];
    isValidSort = false;
    ...
    if(isValidSort){
        ...
        if(filter){
            ...
            for(i=0; i<orders.length; i++){
                if(orders[i].isReviewedByBuyer == filter){
...
function getorders(User){
    return async (req, res, next)=>{
        try {
            ...
            let targetedUser = await User.findById(req.user._id).populate('orders.orderedProduct').exec();
            let orders = targetedUser.orders;
